...
if ( ! function_exists( 'gema_get_posted_on' ) ) :
	...
	function gema_get_posted_on() {
		...
		$byline = sprintf(
            '<span class="byline__by">%s</span> <span class="author vcard"><a class="url fn n" href="%s">%s</a></span>',
			esc_html__( 'by', 'gema' ),
            esc_url( $author_url ),
...
if ( ! function_exists( 'gema_get_first_category' ) ) :
     ...
     * @return string
	 */
	function gema_get_first_category( $post_ID = null) {
		global $wp_rewrite;
...
if ( ! function_exists( 'gema_get_post_format_link_url' ) ) :
     ...
     * @return string URL
     */
    function gema_get_post_format_link_url() {
 ...
 * @return string|array
 */
function gema_audio_attachment( $extra_details = false ) {
 ...
 * @return string
 */
function gema_video_attachment() {
    return hybrid_media_grabber( array( 'type' => 'video', 'split_media' => true ) );
}

if ( ! function_exists( 'gema_is_visual_media' ) ) :

	function gema_is_visual_media( $post_format, $media_details ) {
		$visual_media = true;
...
if ( ! function_exists( 'gema_get_post_format_first_image' ) ) :

    function gema_get_post_format_first_image() {
        global $post;
...
if ( ! function_exists( 'gema_post_meta' ) ) {
    function gema_post_meta() {
        $meta = gema_get_post_meta();
...
if ( ! function_exists( 'gema_get_post_meta' ) ) {
    /**
     * Get all the needed meta for a post.
     *
     * @return array
     */
    function gema_get_post_meta() {
        ...
        $meta['category'] = gema_get_first_category() . sprintf(
...
function pixelgrade_footer_get_copyright_content() {
	$copyright_text = pixelgrade_option( 'copyright_text', sprintf( ' %1$s <span>'. esc_html__( 'by', 'gema' ) .'</span> %2$s',
		'<a href="https://pixelgrade.com/themes/gema/" title="'. esc_attr__( 'GEMA - A Journal-Inspired WordPress Theme', 'gema' ) .'" rel="theme">'. esc_html__( 'Gema Theme', 'gema' ) .'</a>',
		'<a href="https://pixelgrade.com" title="'. esc_attr__( 'The Pixelgrade Website', 'gema' ) .'" rel="designer">Pixelgrade</a>' ) );
