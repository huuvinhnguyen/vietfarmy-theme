 ...
 */
function gema_setup_pixelgrade_care() {
	/*
	 * Declare support for Pixelgrade Care
	 */
	add_theme_support( 'pixelgrade_care', array(
			'support_url'   => 'https://pixelgrade.com/docs/gema/',
			'changelog_url' => 'https://wupdates.com/gema-changelog',
		)
	);
}
add_action( 'after_setup_theme', 'gema_setup_pixelgrade_care', 10 );

function gema_hide_pixelgrade_care_menu_item() {
	?>
	<style>
...
add_action( 'admin_footer', 'gema_hide_pixelgrade_care_menu_item', 10 );

/**
 * After the first theme activation we need to ensure that Pixelgrade Care is installed
 * So we add a transient which will be handled by the `gema_admin_redirect_to_pixcare_install_once` action
 */
function gema_force_redirect_to_pixcare_install_once() {
	...
	if ( empty( $plugin_version ) ) {
		set_transient( '_gema_activation_redirect', 1 );
	}
}
add_action( 'after_switch_theme', 'gema_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function gema_admin_redirect_to_pixcare_install_once() {
	if ( ! get_transient( '_gema_activation_redirect' ) ) {
		return;
	}
	delete_transient( '_gema_activation_redirect' );
...
add_action( 'admin_init', 'gema_admin_redirect_to_pixcare_install_once' );

function gema_add_pixcare_install_page() {
	add_theme_page(  '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'gema_add_pixcare_install_page' );

function gema_pixcare_install_page() {
	 ...
	 */
	$admin_title = apply_filters( 'admin_title', esc_html__( 'Pixelgrade Care &rsaquo; Installer', 'gema' ), '' );
			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'gema' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'gema' ),
								esc_html__( 'Pixelgrade Care&#174;', 'gema' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You\'re in great hands, no doubt about that!', 'gema' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care&#174;', 'gema' ) ?></h5>
									<p class="box__text"></p>
								</div>
							</div>
						</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'gema' ) ?></a>
				</div>
			</div>
		</div>
	</div>
        ...
        $(function () {
		  ...
		  */
          if ( pluginStatus === 'missing' ) {
            $status.text("<?php esc_html_e( 'Installing...', 'gema' ) ?>");
            wp.ajax.settings.url = "<?php echo esc_url_raw( $install_url ) ?>"
          } else if ( pluginStatus === 'installed' ) {
            $staus.text("<?php esc_html_e( 'Activating...', 'gema' ) ?>");
          ...
          wp.ajax.send({type: 'GET'}).always(function (response) {
            ...
            installedSuccessfully = responseText.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'gema' ); ?></p>')
            ...
            folderAlreadyExists = responseText.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'gema' ); ?></p>')
            ...
            if (installedSuccessfully !== -1) {
              ...
              $status.text('<?php esc_html_e( 'Activating...', 'gema' ) ?>') ;
              ...
              wp.ajax.send({type: 'GET'}).always(function (response) {
                ...
                if (activatedSuccessfully !== -1 || noActionTaken !== -1) {
                  ...
                  $status.text('<?php esc_html_e( 'Unfortunately something went wrong. We are sorry, but you will need to install the plugin manually. Redirecting you to the appropriate page...', 'gema' ); ?>')
...
add_action( 'admin_init', 'gema_pixcare_install_page' );
